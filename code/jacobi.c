...
int main(int argc, char *argv[])
{
    ...
    DMR_INIT(initialize_data(&Temperature,&Temperature_last, world_size, world_rank, rows), \
             recv_expand(&Temperature, &Temperature_last, &rows, &it), \
             recv_shrink(&Temperature, &Temperature_last, &rows, &it));
    DMR_Inhibit_iter(2);            // DMR is only called every 2 cycles
    DMR_Set_parameters(1, 4, 0, 2); // min processors, max processors, without function, powers of 2

    // DMR_it is initialized to 0 internally
    while(dt_global > MAX_TEMP_ERROR && DMR_it <= STEPS)
    {
        dt_global = compute(Temperature, Temperature_last, world_size, world_rank, rows, &it);
        DMR_it++;
        DMR_RECONFIGURATION(send_expand(Temperature_last, rows, it), send_shrink(Temperature_last, rows, it));
